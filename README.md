...
- 输入框按 `↑` / `↓` 可以翻回这段对话里问过的问题，和终端里的历史键一样：`↑` 从最新的问题往回走，
  `↓` 再往前走，走到最新问题之后再按一次 `↓` 就回到你**正在打的那半句**（不是清空）。编辑过翻出来的
  问题就当成你自己写的文本，下一次按方向键不会把修改丢掉。多行提问时方向键先照常移动光标，光标到了
  最上/最下一行才接管为历史键；这段对话没问过任何问题时，方向键仍然是普通的光标移动。
...
- 模型配置新增「登录官方模型（pi 授权）」：可选择 ChatGPT Plus/Pro（OpenAI Codex）、Claude Pro/Max、GitHub Copilot 或 OpenRouter，用 pi 的官方 provider 完成 OAuth/设备码登录，然后选择模型并添加配置、点击「保存全部配置」即可使用；不需要手动填写 API Key。Codex 默认使用适合远程主控的设备码登录；其它需要网页回调的授权，如果浏览器无法访问主控的 localhost，可粘贴跳转后的完整回调地址。支持重新登录、退出登录和过期凭据自动刷新。授权令牌作为管理员共享的全局模型凭据加密保存在主控数据库中，不返回浏览器，也不会写入 pi 的 `auth.json`；OAuth 登录/登出仅管理员可用。官方模型请求走 pi 的原生 provider 协议（包括工具调用），不是把订阅令牌塞进普通 OpenAI 兼容地址；官方地址与模型均是服务端白名单，不允许修改。Claude 订阅的第三方调用可能按 extra usage 单独计费、OpenRouter 授权会创建消耗账户余额的 API Key，具体权限及费用由各提供商决定。其它官方模型仍可用下面的 API Key 方式配置；模型列表随项目锁定的 pi-ai 版本更新，不承诺实时拉取在线目录。
...
需要 Node.js 22.19 或更高版本（官方模型依赖的 pi-ai 要求 ≥22.19）。Linux 裸机还需 GLIBC 2.38+；较旧发行版请使用项目提供的 Docker 镜像。
