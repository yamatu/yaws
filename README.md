...
- “AI 助手”既是工作区的聊天标签页，也是可悬浮在终端、文件等任何标签页之上的附加面板（右下角按钮展开/收起）。悬浮面板可拖动标题栏移动到任意位置，右下角可调整大小，位置和尺寸记在浏览器中（双击标题栏回到默认右下角），拖动/缩放始终限制在可视区域内，手机上也不会盖住触屏按键栏。对话按机器保存，可继续提问或删除。
