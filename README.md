...
- 助手的多机模式：对话框底部的「主机」按钮显示本轮可用主机数（如 `主机 1/3`），点开是一份勾选列表。当前机器的标签页就是主机器（列表里显示“主”，不会重复出现），勾上其它机器后，本轮提问的主机集合就是“主机器 + 勾选项”；选中的主机会在输入框上方以标签列出，勾选状态随对话一起保存在服务端，重新打开这段对话时会恢复。因此：想在 `web-1` 上操作却顺手在 `db-1` 的标签页里提问，不必切标签——把 `db-1` 勾上即可。
